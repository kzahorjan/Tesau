...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    --sp-42:42px; --sp-48:48px; --sp-52:52px; --sp-58:58px; --sp-68:68px;
  }

  body { font-family:'Inter',sans-serif; background:var(--purple-400); color:var(--color-text-primary); }

  /* ── Typography ── */
  .t-label      { font-family:'Inter',sans-serif; font-size:14px; font-weight:500; line-height:1.4; letter-spacing:0.02em; color:var(--color-text-primary); }
  .t-small-label{ font-family:'Inter',sans-serif; font-size:12px; font-weight:500; line-height:1.4; letter-spacing:0.02em; color:var(--color-text-secondary); text-transform:uppercase; }
  .slider-wrapper .t-small-label { color:var(--purple-200); }
  .t-body       { font-family:'Inter',sans-serif; font-size:16px; font-weight:400; line-height:1.5; color:var(--color-text-primary); }
  .t-small-body { font-family:'Inter',sans-serif; font-size:14px; font-weight:400; line-height:1.5; color:var(--color-text-primary); }
  .t-button     { font-family:'Inter',sans-serif; font-size:16px; font-weight:500; line-height:1;   color:var(--color-text-primary); pointer-events:none; }
  ...
  .btn-base:focus-visible { outline: 2px solid #d100d1; outline-offset: 3px; }

  .btn-primary  { background: linear-gradient(135deg, #e70bdd 0%, var(--purple-500) 100%); }
  .btn-primary::after {
  ...
  /* ── Slider ── */
  .slider-wrapper { display:flex; flex-direction:column; gap:var(--sp-8); width:100%; }
  .slider-header  { display:flex; justify-content:space-between; align-items:center; }
  .slider-outer-wrap { position:relative; }
  .slider-stroke-svg {
  ...
  .slider-thumb { width:4px; height:24px; background:var(--purple-500); border-radius:2px; flex-shrink:0; }
  .slider-input { position:absolute; inset:0; opacity:0; cursor:pointer; width:100%; height:100%; z-index:2; }
  ...
  @keyframes analysingPulse { 0%,100%{opacity:1} 50%{opacity:0.55} }

  .prompt-outer-wrap { position:relative; width:889px; max-width:100%; }
  .prompt-stroke-svg {
    position:absolute; inset:0; width:100%; height:100%;
    pointer-events:none; overflow:visible;
  }
  ...
  .checkbox-outer-wrap { position:relative; width:24px; height:24px; flex-shrink:0; }
  .checkbox-stroke-svg {
    position:absolute; inset:0; width:100%; height:100%;
    pointer-events:none; overflow:visible;
  }
  ...
  .section-title {
    ...
    color:var(--color-text-secondary); margin-bottom:20px; padding-bottom:8px;
  ...
  .demo-label { font-size:11px; color:var(--color-text-secondary); margin-bottom:8px; letter-spacing:0.06em; text-transform:uppercase; }
  ...
  .panel-box { background:var(--color-background-panel); border-radius:12px; padding:20px; border:1px solid rgba(255,255,255,0.06); height:120px; }
`;

/* ─────────────────────────────────────────────
   SVG GRADIENT STROKE OVERLAY
   Draws a pixel-perfect 1px rounded-rect stroke
   with a linearGradient on top of any element.
   r = corner radius of the element beneath it.
   Colors default to the white 20%→30% spec.
───────────────────────────────────────────── */
let _gsId = 0;
const GradientStroke = ({
  r = 10,
  x1 = "0%", y1 = "0%", x2 = "100%", y2 = "100%",
  stop1 = "rgba(255,255,255,0.35)",
  stop2 = "rgba(255,255,255,0.05)",
  className = "",
}) => {
  const id = useRef(`gs-${++_gsId}`).current;
  return (
    <svg className={className} style={{ position:"absolute", inset:0, width:"100%", height:"100%", pointerEvents:"none", overflow:"visible" }}>
      <defs>
      ...
      {/* strokeWidth 1, inset by 0.5px so the full stroke stays inside */}
      <rect
        x="0.5" y="0.5"
...
/* Purple-tinted stroke variant for prompt input */
const PromptStroke = () => (
  <GradientStroke
    r={10}
    stop1="rgba(174,112,171,0.55)"
    stop2="rgba(174,112,171,0.10)"
  />
);
...
export const Checkbox = ({ label = "Option", defaultChecked = false }) => {
  ...
  return (
    <label className="checkbox-wrap" onClick={() => setChecked(c => !c)}>
      <div className="checkbox-outer-wrap">
        <div className={`checkbox-box ${checked ? "checked" : ""}`}>
          {checked && <TickIcon />}
        </div>
        {/* Only show stroke when unchecked */}
        {!checked && <GradientStroke r={4} />}
      </div>
      <Body>{label}</Body>
    </label>
  );
      ...
      <div style={{ padding:"48px", maxWidth:1200, margin:"0 auto" }}>
          ...
          <div className="swatch-row">
            {[
              ...
              <div key={token} style={{ textAlign:"center", minWidth:64 }}>
                <div className="swatch" style={{ background:hex, width:48, height:48 }} />
        ...
        <div className="demo-section">
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div><div className="demo-label">Search</div><PromptInput state="search" /></div>
            <div><div className="demo-label">Analysing</div><PromptInput state="analysing" /></div>
            <div><div className="demo-label">Typing</div><PromptInput state="typing" /></div>
          </div>
        </div>
        ...
        <div className="demo-section">
          ...
          <div className="layout-grid" style={{ marginBottom:20 }}>
            <div className="layout-main panel-box" style={{ display:"flex", alignItems:"center", justifyContent:"center" }}><SmallLabel>Main workspace · cols 1–4</SmallLabel></div>
            <div className="layout-sidebar panel-box" style={{ display:"flex", alignItems:"center", justifyContent:"center" }}><SmallLabel>Sidebar · cols 5–6</SmallLabel></div>
          </div>
          <div className="demo-label" style={{ marginTop:16 }}>Modal: 4-col centred (cols 2–5)</div>
          <div className="layout-grid">
            <div className="layout-modal panel-box" style={{ display:"flex", alignItems:"center", justifyContent:"center" }}><SmallLabel>Modal · cols 2–5 · centred</SmallLabel></div>
          </div>
        </div>
